...
public class InvitationController(IInvitationService invitationService, IEmailSender<ApplicationUser> emailSenderApplication) : ControllerBase {
    ...
    [HttpPost]
    public async Task<ActionResult<InvitationDto>> Create([FromQuery] string email, [FromQuery] string invitedByUserId, [FromQuery] int? leagueId = null, [FromQuery] bool isLeagueOwner = false, [FromQuery] string? baseUrl = null)
    {
        var invitation = await invitationService.CreateInvitationAsync(email, invitedByUserId, leagueId, isLeagueOwner, baseUrl);
