...
//
// Fails closed: a week with no SpreadLockDatetime configured yet is treated as "not locked" —
// blocked, not allowed through by default. The only way past this is an explicit "force" flag in
// the triggering JobDataMap, set by JobManagerController's admin endpoints for a genuine outage
// where spreads need to go out immediately regardless of the configured lock time.
internal static class SpreadLockGuard {
    ...
    public static bool ShouldSkip(DateTime? lockDatetime, DateTime now, IJobExecutionContext context) {
        ...
        return lockDatetime is null || now < lockDatetime;
    }
