...
- **Before opening ANY PR — bead-tracked or not, hotfix or not** — run the full test suite, `/simplify`, and `/feature-dev:code-review` (see "Before Opening a PR" under Bead Standards below for the exact steps). This is not conditional on the work having a bead: an ad-hoc fix opened straight from a live session needs the same gate as bead-driven work, since that's exactly when review is easiest to skip under time pressure.
- **Add a `Client.React/CHANGELOG.md` entry for any user-facing or notable change** before opening a PR — one line under today's `## YYYY-MM-DD` heading (create it if today's doesn't exist yet). Rendered on the admin-only Changelog page (`/admin/changelog`). Skip it for purely internal changes (refactors, test-only changes, CI/tooling tweaks) that no admin needs to know shipped. The file only supports `## ` headings and single-line `- ` bullets — no bold, links, or multi-line bullets (a canary test, `changelogFormat.test.ts`, enforces this).
...
- Before a `dev`→`main` cutover (season launch or major release), run `/full-test-local` — the local real-mode prod-readiness gate (Docker Postgres, real ESPN calls, no shared staging environment needed). Email is intentionally suppressed in every non-production environment (`GoogleEmailSender`, gated on `IWebHostEnvironment.IsDevelopment()` for local runs and `RAILWAY_ENVIRONMENT_NAME` for the deployed dev environment) — email-triggering steps are verified via the backend log's `📧 Email suppressed` line (which includes the full body/links), not a real inbox. After the merge deploys, run `/prod-smoke-test` for the handful of checks that need a real HTTPS host (cookie flags, CORS enforcement, deploy freshness); its live game-week section is a human-executed checklist, not agent-driven.
...
4. **`Client.React/CHANGELOG.md` entry** — one line under today's date, for anything user-facing or notable (see Branch Rules above)
...
Sport is determined by subdomain: `localhost:5173` / `ivleague.xyz` → NFL; `cfb.localhost:5173` / `cfb.ivleague.xyz` → CFB.
...
### Infra MCP connectors: Neon, Railway, Vercel
This session has live MCP access to the three hosting/DB providers, useful for debugging real prod issues (e.g. a user-reported bug that needs checking against actual data) rather than guessing from code alone:
- **Neon** (`mcp__Neon__*`) — prod Postgres. `list_projects` requires an `org_id` that isn't obvious from the tool itself; get it first via `list_organizations` (org "Frizat", `org-calm-forest-92763085`). The app's project is `ivleague` (`shiny-scene-21715580`). Reading/querying prod data (read-only SQL) to investigate a live bug report is fine when the user asks for it or the task calls for it; never run destructive SQL without explicit confirmation (the tool itself also enforces this).
- **Railway** (`mcp__Railway__*`) — hosts the `ivleague` service (project `989b2756-1fc0-41c3-8f97-874e58beb81a`), both `development` and `production` environments. Reading secrets/variables (`list-variables`) is gated by a permission classifier independent of this file's rules — expect a denial even after the user confirms in chat; only a settings permission rule (not conversational approval) allows it. Prefer Neon MCP directly for DB access instead of round-tripping through Railway's `DATABASE_URL`.
- **Vercel** (`mcp__Vercel__*`) — frontend hosting/deploys, preview URLs, deployment/runtime logs.

## CRITICAL: `ADMIN_PASSWORD` — never let an example placeholder become a real deployed value
`UserManagerJob.SyncAdminPassword` (`Server/Jobs/UserManagerJob.cs`) writes the real `Administrator` account's password to match `ADMIN_PASSWORD` **only the first time that account is created** (a one-shot Quartz trigger ~2 min after boot — it is not on-demand re-triggerable via the Job Manager API once it has fired once, since Quartz drops a non-durable one-shot job after its only trigger completes; re-running it requires a full service restart/redeploy). This already caused a real incident: `.env.backend.example`'s `ADMIN_PASSWORD` placeholder had been copy-pasted verbatim into Railway's dev environment variable, so the real admin account's live password was identical to a value published in this public repo's source/example files. Never set a real environment's `ADMIN_PASSWORD` to any value that appears anywhere in tracked source (including `.env.backend.example` and `DemoDataSeeder.cs`'s demo-user password) — always generate a unique value per real environment.

**Second incident (2026-08-22, `frizat-wyo`):** the job used to re-run this sync unconditionally on *every* startup, not just first-ever creation — so a routine `dev`→`main` redeploy silently reset the repo owner's own deliberately-changed password back to the `ADMIN_PASSWORD` Railway variable, and they got locked out of their own account via ASP.NET Identity's failed-login lockout trying their (now-stale) remembered password. Fixed: `SyncAdminPassword` now only overwrites the hash when the account was just bootstrapped this run, or when `ADMIN_FORCE_PASSWORD_SYNC=true` is explicitly set (this forced path logs a distinct `Warning`, not just the routine `Information` line, precisely because it's the one case that still overwrites an existing password). **To rotate/recover** an existing admin's password: update the `ADMIN_PASSWORD` Railway variable, set `ADMIN_FORCE_PASSWORD_SYNC=true` on that environment, restart/redeploy the service (not the one-shot job API) to force the sync, then **unset `ADMIN_FORCE_PASSWORD_SYNC`** again — leaving it `true` recreates the original bug on every subsequent restart. Verify via a login attempt with the old value (must fail) before considering it done. **If recovering from an active lockout** (5 failed attempts trips a 15-minute `DefaultLockoutTimeSpan`, `Program.cs` ~line 128): resyncing the password hash does not clear `LockoutEnd`/`AccessFailedCount` — a login with the newly-correct password will still be rejected until the lockout window itself elapses, so wait it out (or clear the lockout fields directly) before concluding the resync didn't work.
